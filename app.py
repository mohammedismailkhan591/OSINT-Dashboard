...
if st.button("🔍 Start Investigation", use_container_width=True):
    ...
    if not target:
        ...
        target_details = {
            ...
            "Redirects": str(len(response.history)),
            "Status": str(response.status_code),
            "HTTP Method": "GET",
        }

        target_df = pd.DataFrame(
            list(target_details.items()),
        ...
        if response.status_code == 200:
            ...
            st.warning(
                f"Website responded with status {response.status_code}"
            )
        ...
        if start != -1 and end != -1:
            ...
            title = " ".join(
                response.text[start + 7:end].split()
            )
        ...
        chart_data = pd.DataFrame(
            {
                ...
                "Score": [
                    ...
                    1 if response.url.lower().startswith(
                        "https://"
                    ) else 0,
        ...
        try:
            ...
            st.warning(
                "WHOIS information is not available."
            )
